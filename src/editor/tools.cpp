...
CHandTool::CHandTool( QObject *parent ) : CTool( parent ) { m_selectedBlockPos = QVector3D( 0, 0, 0 ); }
		...
		if ( event->button() == Qt::LeftButton )
		{
			if ( m_selectedBlockNormal != QVector3D( 0, 0, 0 ) )
			{
				...
				if ( dlg->exec() == QDialog::Accepted )
				{
					...
					view->m_chunk->set( m_selectedBlockPos.x(), m_selectedBlockPos.y(), m_selectedBlockPos.z(), id,
										meta );
					view->m_chunk->rebuildModel();
					view->update();
				}
