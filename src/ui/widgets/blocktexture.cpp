...
int BlockTexture::heightForWidth( int width ) const
{
    return width;
}

bool BlockTexture::hasHeightForWidth() const
{
    return true;
}
...
void BlockTexture::mousePressEvent( QMouseEvent *event )
{
    emit clicked( );
}

void BlockTexture::mouseDoubleClickEvent( QMouseEvent *event )
{
    emit doubleClicked( );
}
...
uint16_t BlockTexture::getBlockId()
{
    return m_id;
}