...
bool Meegreef1Format::Save( CWorld *world, QString file ) { return false; }
...
bool VoxelFormatYeah::Load( CWorld *world, QString filePath )
{
	...
	for ( QString chunkFile : chunkFiles )
	{
		...
		// chunk data is sizeX * sizeY * sizeZ * 4 bytes, and then another sizeX * sizeY * sizeZ * 2 bytes for the
		// lighting.
		if ( data.size() < ( chunkSizes.x * chunkSizes.y * chunkSizes.z * 4 ) )
		{ // TODO: lighting + chunkSizes.x * chunkSizes.y * chunkSizes.z * 2)) {
			qWarning() << "VoxelFormatYeah: Chunk file too small" << chunkFile;
			continue;
		}
...
bool VoxelFormatYeah::Save( CWorld *world, QString filename )
{
	...
	toml::table root = toml::table{ {
		{ "meta", toml::table{ {
					  ...
					  { "chunk_sizes",
						toml::array{ world->getChunkSize().x, world->getChunkSize().y, world->getChunkSize().z } },
				  } } },
